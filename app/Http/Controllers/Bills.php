    ...
    */
        ...
        else
        {
            ...
            if($bill_id)
            {
                if(count($request->particular) > 0)
                {
                    ...
                    {
                        DB::commit();
                        return redirect('/bill-view/'.Crypt::encrypt($bill_id))->withSuccess('Success');
                    }
                    else
                    {
                        return redirect('/bill/')->withErrors('Please fill the form correctly.Price should not be zero.');
                    }
                }
                DB::commit();
                return redirect('/bill-view/'.Crypt::encrypt($bill_id))->withSuccess('Success');
    ...
    public function particularTableStore($request,$particular_array,$description_array,$quantity_array,$rate_array,$units_array,$bill_id,$itemType_array)
    {
        ...
        if($total > 0)
        {
            ...
            for($i=0;$i<$total;$i++)
            {
                $price = round($quantity_array[$i] * $rate_array[$i],2);
                ...
                $particular->quantity = round($quantity_array[$i],2);
                $particular->rate = round($rate_array[$i],2);
        ...
        else
        {
            ...
            if($bill_id)
            {
                ...
                if(count( $request->particular  ) > 0)
                {
                    ...
                    {
                        return redirect('/bill-view/'.Crypt::encrypt($bill_id))->withErrors('Please fill the form correctly.Price should not be zero.');
                    }
                }
                DB::commit();
                return redirect('/bill-view/'.Crypt::encrypt($bill_id))->withSuccess('Success');
