...
<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
    ...
    <ul class="nav navbar-right top-nav">
        <?php 
            if(Request::segment(1) == 'bill-view')
            {
                ?>
                    <li>
                        <a href="{{url('/bill-edit/'.Crypt::encrypt($bill_detail->id))}}"><button class="btn btn-info btn-xs">Edit</button></a>
                    </li>
                <?php
            }
        ?>
        <?php 
            if(Request::segment(1) == 'bill-view' || Request::segment(1) == 'customer-report')
            {
                ?>
                    <li>
                        <a href=""><button class="btn btn-success btn-xs" id="print_bill">Print</button></a>
                    </li>
                <?php
            }
        ?>
