					...
					if($items_reports)
					{
						foreach($items_reports as $items_repor)
						{
							...
							if(!$IsBillCashOnly)
							{
								?>
								<tr>
									<td><?php if(!in_array($items_repor->entry_date, $items_report_date)){echo $items_repor->entry_date;} ?></td>
									<td><?php echo app('App\Http\Controllers\Items')->getItemNepaliName($items_repor->particular_id).' '.$items_repor->quantity.' '.app('App\Http\Controllers\Units')->getUnitsNepaliName($items_repor->units_id).' ('.$items_repor->description.') => Rs. '.$items_repor->rate.'*'.$items_repor->quantity; ?></td>
									<td><?php echo $items_repor->debit; ?></td>
									<td><?php echo $items_repor->credit; ?></td>
								</tr>
								<?php
								array_push($items_report_date, $items_repor->entry_date);
							}
						}
					}
				?>
							...
							if($cash_repor->cash_received > 0 || $cash_repor->cash_given > 0 )
							{
								?>
									<tr>
										...
										<td><?php if($cash_repor->cash_received > 0){ echo "रकम प्राप्त  ".$cash_repor->description_for_cash_only;}else{echo "रकम दिएको ".$cash_repor->description_for_cash_only;} ?></td>
...
@section('script')
	<script type="text/javascript">
		$(document).ready(function(){
		    ...
		    $('#customer_report_item_wrapper').addClass('table-responsive');
		})
	</script>
@endsection
